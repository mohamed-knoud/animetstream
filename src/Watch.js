...
useEffect(() => {
	...
	console.log(response2.data);
		      if (Hls.isSupported()) {
			      const hls = new Hls();
			      hls.loadSource("https://proxy-ryan.vercel.app/cors?url="+response2.data.data.sources[0].url);
		    ...
		    if (videoRef.current) {
		      videoRef.current.src = "https://proxy-ryan.vercel.app/cors?url="+response2.data.data.sources[0].url;
		    }
} catch (error) {
	console.error(error);
}
